...
class ProductRestControllerTest(
    ...
    fun `상품 목록을 조회할 수 있다`() {
        ...
        Given {
            ...
            body("content[0].categoryId", equalTo(commandThree.categoryId.toInt()))
            body("content[0].nameKo", equalTo(commandThree.nameKo))
            body("content[0].thumbnailUrl", equalTo(commandThree.images.first().imageUrl))
            body("content[1].id", notNullValue())
            body("content[1].categoryId", equalTo(commandTwo.categoryId.toInt()))
            body("content[1].nameKo", equalTo(commandTwo.nameKo))
            body("content[1].thumbnailUrl", equalTo(commandTwo.images.first().imageUrl))
            body("content[2].id", notNullValue())
            body("content[2].categoryId", equalTo(commandOne.categoryId.toInt()))
            body("content[2].nameKo", equalTo(commandOne.nameKo))
            body("content[2].thumbnailUrl", equalTo(commandOne.images.first().imageUrl))
            body("page", equalTo(1))
    ...
    fun `상품을 검색할 수 있다`() {
        ...
        Given {
            ...
            statusCode(HttpStatus.SC_OK)
            body("content[0].id", equalTo(savedProductThree.id.toInt()))
            body("content[0].categoryId", equalTo(savedProductThree.categoryId.toInt()))
            body("content[0].nameKo", equalTo(savedProductThree.nameKo))
            body("content[0].thumbnailUrl", equalTo(savedProductThree.images.first().imageUrl))
            body("content[1].id", equalTo(savedProductOne.id.toInt()))
            body("content[1].categoryId", equalTo(savedProductOne.categoryId.toInt()))
            body("content[1].nameKo", equalTo(savedProductOne.nameKo))
            body("content[1].thumbnailUrl", equalTo(savedProductOne.images.first().imageUrl))
            body("page", equalTo(1))
            body("size", equalTo(10))
            body("last", equalTo(true))
            log().all()
